...
const JPushSdk = require('jpush-sdk');
...
class Jpush {
  ...
  */
  ...
  // async sendAllValidate(content, extras = {}) {
  //   return await this.sendValidate('all', content, extras);
  // }
  ...
  */
  ...
  async sendValidate() {
    const { appKey, masterSecret, isDebug } = this.config;
    const client = JPushSdk.buildClient({ appKey, masterSecret, isDebug });
    return new Promise(function(resolve, reject) {
      client.push().setPlatform('all').setAudience('all')
        .setNotification('Base.ALERT')
        .sendValidate(function(err, res) {
          if (!err && res) {
            // done();
            console.log('sendValidate action over');
          }
        });
    });
  }
